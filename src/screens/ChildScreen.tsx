...
const statuses: MedicalItem["replacementStatus"][] = [
  "OK",
  "Due soon",
  "Requested",
  "Replaced",
...
export function ChildScreen() {
  ...
  const [profile, setProfile] = useState<ChildProfileInput>({
    ...
    className:
      state.child.className === "Class not added" ? "" : state.child.className,
    allergies: state.child.allergies,
    clothingSize:
      state.child.clothingSize === "Size not added"
        ? ""
        : state.child.clothingSize,
    shoeSize: state.child.shoeSize === "Not added" ? "" : state.child.shoeSize,
  });
  const [allergiesText, setAllergiesText] = useState(
    state.child.allergies.join(", "),
  );
  const [medicalForm, setMedicalForm] = useState<MedicalForm>(() =>
    freshMedicalForm(state.child.currentHousehold),
  );

  const editingMedical = state.medicalItems.find(
    (item) => item.id === editingMedicalId,
  );
  const otherParent = members.some((member) => member.parentLabel === "Mum")
    ? "Dad"
    : "Mum";
  ...
  const openProfile = () => {
    setProfile({
      name: state.child.name,
      school:
        state.child.school === "School not added" ? "" : state.child.school,
      className:
        state.child.className === "Class not added"
          ? ""
          : state.child.className,
      allergies: state.child.allergies,
      clothingSize:
        state.child.clothingSize === "Size not added"
          ? ""
          : state.child.clothingSize,
      shoeSize:
        state.child.shoeSize === "Not added" ? "" : state.child.shoeSize,
    });
  ...
  const saveMedical = async () => {
    ...
    if (
      !editingMedical?.photoPath &&
      !editingMedical?.photoUrl &&
      !medicalForm.photo
    ) {
      Alert.alert(
        "Picture needed",
        "Take or choose a clear picture of the device or label.",
      );
      return;
    }
    setBusy(true);
    try {
      if (editingMedicalId)
        await updateMedicalItem(editingMedicalId, medicalInput());
  ...
  const removeMedical = () => {
    ...
    Alert.alert(
      "Remove this medical item?",
      "Its location and expiry record will be deleted from both homes.",
      [
  ...
  const makeInvite = async () => {
    ...
    try {
      ...
      Alert.alert(
        ...
        errorMessage(
          caught,
          "Run the HomeBridge v0.7 database patch and try again.",
        ),
      );
    } finally {
      setBusy(false);
  ...
  const removeHouseholdMember = (userId: string, displayName: string) => {
    Alert.alert(
      ...
      `${displayName} will immediately lose access to this household.`,
      [
        { text: "Cancel", style: "cancel" },
        {
          text: "Remove",
          style: "destructive",
          onPress: () => {
            removeMember(userId).catch((caught) =>
              Alert.alert("Could not remove member", errorMessage(caught)),
            );
          },
        },
      ],
  ...
  return (
    <>
      ...
      >
        <AppHeader
          title={state.child.name}
          subtitle="Important information shared between both homes"
        />
        <Pressable onPress={openProfile}>
          <Card style={styles.profileCard}>
            ...
            <Text style={styles.school}>
              {state.child.school} · {state.child.className}
            </Text>
            <View style={styles.profilePills}>
              <Pill label={state.child.clothingSize} tone="blue" />
              <Pill label={`Shoes ${state.child.shoeSize}`} tone="blue" />
            </View>
        ...
        <SectionHeader
          ...
          action="Add medical item"
          onAction={openAddMedical}
        />
        {state.medicalItems.length ? (
          state.medicalItems.map((item) => {
              ...
              <Pressable key={item.id} onPress={() => openMedical(item)}>
                <Card style={styles.medicalCard}>
                  ...
                  <View style={styles.medicalCopy}>
                    ...
                    <Text style={styles.medicalMeta}>
                      {item.quantity} device{item.quantity === 1 ? "" : "s"} ·{" "}
                      {item.location}
                      {!item.photoUrl ? " · Photo required" : ""}
                    </Text>
                    <Text
                      style={[styles.expiry, days <= 60 && styles.expiryUrgent]}
                    >
                      Expires {formatDay(item.expiryDate)} · {days} days
                    </Text>
                    <Text style={styles.checked}>
                      Last physically checked {formatDay(item.lastCheckedAt)}
                    </Text>
                  </View>
                  <Pill
                    label={item.replacementStatus}
                    tone={item.replacementStatus === "OK" ? "green" : "rose"}
                  />
                </Card>
              </Pressable>
            );
          ...
          <EmptyState
            emoji="🩺"
            title="No medical items added"
            body="Add each EpiPen or other essential separately, including a clear picture, location and expiry."
          />
        )}
        ...
        <Card>
            ...
            <React.Fragment key={member.userId}>
              ...
              <View style={styles.parentRow}>
                <View style={styles.parentAvatar}>
                  <Text style={styles.parentInitials}>
                    {member.displayName.slice(0, 1).toUpperCase()}
                  </Text>
                </View>
                <View style={styles.parentCopy}>
                  <Text style={styles.parentName}>{member.displayName}</Text>
                  <Text style={styles.parentRole}>
                    {member.parentLabel || member.role}
                  </Text>
                </View>
                <View style={styles.parentRight}>
                  <Text style={styles.parentStatus}>
                    {member.role === "owner" ? "Household owner" : "Connected"}
                  </Text>
                  {workspaceRole === "owner" && member.role !== "owner" ? (
                    <Pressable
                      onPress={() =>
                        removeHouseholdMember(member.userId, member.displayName)
                      }
                    >
                ...
                label={
                  busy
                    ...
                    : pendingInvites.length
                      ? `Create another ${otherParent} code`
                      : `Invite ${otherParent}`
                }
          ...
          {mode === "live" && members.length >= 2 ? (
            <Text style={styles.connectedNote}>
              Both parents are connected to this household.
            </Text>
          ) : null}
        </Card>
        ...
        <Card>
          <SettingRow
            ...
            body="Only authenticated, invited household members can access these records."
          />
          <SettingRow
            icon="🧾"
            title="Activity history"
            body="Important changes retain who made them and when."
          />
          <SettingRow
            icon="📴"
            title="Cached on this phone"
            body={
              mode === "live"
          ...
          {syncError ? (
            <View style={styles.syncError}>
              <Text style={styles.syncErrorTitle}>Last sync issue</Text>
            ...
            <SecondaryButton
              ...
              onPress={() =>
                signOut().catch((caught) =>
                  Alert.alert("Could not sign out", errorMessage(caught)),
                )
              }
            />
          )}
      ...
      <BottomSheet
        ...
        title={`Edit ${state.child.name}'s profile`}
      ...
      >
        ...
        <Field
          ...
          onChangeText={(school) =>
            setProfile((current) => ({ ...current, school }))
          }
        ...
        <Field
          ...
          onChangeText={(className) =>
            setProfile((current) => ({ ...current, className }))
          }
        ...
        <Field
          ...
          onChangeText={(clothingSize) =>
            setProfile((current) => ({ ...current, clothingSize }))
          }
        ...
        <Field
          ...
          onChangeText={(shoeSize) =>
            setProfile((current) => ({ ...current, shoeSize }))
          }
      ...
      <BottomSheet
        ...
        title={editingMedicalId ? "Edit medical item" : "Add medical item"}
        onClose={() => setSheet(null)}
      >
        <PhotoField
          required
          photo={medicalForm.photo}
          existingUrl={editingMedical?.photoUrl}
          label="Picture of device or label"
          onChange={(photo) =>
            setMedicalForm((current) => ({ ...current, photo }))
          }
        />
        ...
        <Field
          ...
          onChangeText={(name) =>
            setMedicalForm((current) => ({ ...current, name }))
          }
          placeholder="e.g. EpiPen 0.3 mg"
        />
        <Text style={styles.label}>Location</Text>
        <ChipGrid
          values={locations}
          selected={medicalForm.location}
          onSelect={(location) =>
            setMedicalForm((current) => ({ ...current, location }))
          }
        />
        ...
        >
          <Text style={styles.dateButtonText}>
            {formatDay(medicalForm.expiryDate.toISOString())}
          </Text>
        </Pressable>
        <Text style={styles.label}>Replacement status</Text>
        <ChipGrid
          values={statuses}
          selected={medicalForm.replacementStatus}
          onSelect={(replacementStatus) =>
            setMedicalForm((current) => ({ ...current, replacementStatus }))
          }
        />
        ...
        <Field
          ...
          onChangeText={(notes) =>
            setMedicalForm((current) => ({ ...current, notes }))
          }
          placeholder="Dose, prescription request, storage details…"
        />
        <View style={styles.sheetAction}>
          <PrimaryButton
            label={editingMedicalId ? "Save medical item" : "Add medical item"}
            onPress={() => void saveMedical()}
            busy={busy}
            disabled={!medicalForm.name.trim()}
          />
        </View>
        {editingMedicalId ? (
          <View style={styles.secondaryAction}>
            <DangerButton
              label="Remove medical item"
              onPress={removeMedical}
              disabled={busy}
            />
          </View>
        ) : null}
      ...
      {showExpiryPicker ? (
        <DateTimePicker
          value={medicalForm.expiryDate}
          mode="date"
          onChange={(_event: DateTimePickerEvent, date?: Date) => {
            setShowExpiryPicker(false);
            if (date)
              setMedicalForm((current) => ({ ...current, expiryDate: date }));
          }}
        />
      ) : null}
    </>
