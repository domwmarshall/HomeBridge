...
export function ThingsScreen() {
  ...
  const [form, setForm] = useState<ItemFormState>(() =>
    blankForm(state.child.currentHousehold),
  );
  ...
  const toInput = (): ItemInput => ({
    ...
    minimumAtDad: form.minimumAtDad.trim()
      ? Math.max(0, Number.parseInt(form.minimumAtDad, 10) || 0)
      : undefined,
    minimumAtMum: form.minimumAtMum.trim()
      ? Math.max(0, Number.parseInt(form.minimumAtMum, 10) || 0)
      : undefined,
  ...
  const save = async () => {
    ...
    if (!editingItem?.photoUrl && !editingItem?.photoPath && !form.photo) {
      Alert.alert(
        "Picture needed",
        "Take or choose a picture of the actual item before saving.",
      );
      return;
    }
    setBusy(true);
        ...
        { text: "Cancel", style: "cancel" },
        {
          ...
          onPress: () => {
            ...
            deleteItem(editingId)
              .then(closeForm)
              .catch((caught) =>
                Alert.alert("Could not remove item", errorMessage(caught)),
              )
  ...
  return (
    <>
      ...
      >
        <AppHeader
          title="Things"
          subtitle={`Track ${state.child.name}'s belongings between homes`}
        />
        <Field
          value={search}
          onChangeText={setSearch}
          placeholder="Search uniform, toys, books…"
        ...
        <View style={styles.summaryRow}>
          <Card style={styles.summaryCard}>
            <Text style={styles.summaryNumber}>
              {state.items.filter((i) => i.location === "Dad's house").length}
            </Text>
            <Text style={styles.summaryLabel}>At Dad’s</Text>
          </Card>
          <Card style={styles.summaryCard}>
            <Text style={styles.summaryNumber}>
              {state.items.filter((i) => i.location === "Mum's house").length}
            </Text>
            <Text style={styles.summaryLabel}>At Mum’s</Text>
          </Card>
          <Card style={styles.summaryCard}>
            <Text style={[styles.summaryNumber, { color: colours.danger }]}>
              {state.items.filter((i) => i.location === "Missing").length}
            </Text>
        ...
        <SectionHeader
          title={`${items.length} tracked item${items.length === 1 ? "" : "s"}`}
          action="Add item"
          onAction={openAdd}
        />
        {!items.length ? (
          <EmptyState
            ...
            body="Add an item with a real picture, or change the current filter."
          />
        ) : (
          items.map((item) => (
      ...
      <BottomSheet
        visible={showForm}
        title={editingId ? "Edit item" : "Track a new item"}
        onClose={closeForm}
      >
        <PhotoField
          required
        ...
        <Field
          ...
          placeholder="e.g. Purple swimming bag"
        ...
        <ChipGrid
          values={neededOptions}
          selected={form.neededAt}
          onSelect={(neededAt) =>
            setForm((current) => ({ ...current, neededAt }))
          }
        />
        ...
        <View style={styles.twoFields}>
          <View style={styles.fieldHalf}>
            <Text style={styles.miniLabel}>Dad's</Text>
            <Field
              value={form.minimumAtDad}
              onChangeText={(minimumAtDad) =>
                setForm((current) => ({
                  ...current,
                  minimumAtDad: minimumAtDad.replace(/[^0-9]/g, ""),
                }))
              }
              keyboardType="number-pad"
          ...
          <View style={styles.fieldHalf}>
            <Text style={styles.miniLabel}>Mum's</Text>
            <Field
              value={form.minimumAtMum}
              onChangeText={(minimumAtMum) =>
                setForm((current) => ({
                  ...current,
                  minimumAtMum: minimumAtMum.replace(/[^0-9]/g, ""),
                }))
              }
              keyboardType="number-pad"
              placeholder="Optional"
        ...
        <Field
          ...
          placeholder="Colour, size, identifying marks, what belongs with it…"
        />
        ...
        <View style={styles.secondaryAction}>
          <SecondaryButton label="Cancel" onPress={closeForm} disabled={busy} />
        </View>
      </BottomSheet>
    </>
  );
