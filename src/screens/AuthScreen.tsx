...
export function AuthScreen({ onUseDemo }: { onUseDemo: () => void }) {
  ...
  const submit = async () => {
    if (
      ...
      Alert.alert(
        "Check the details",
        "Use a valid email, a password of at least eight characters, and your name.",
      );
      return;
    }
    setBusy(true);
    try {
      if (mode === "signin") await signIn(email, password);
        ...
        if (result.needsConfirmation) {
          Alert.alert(
            "Check your email",
            "Open the confirmation email from Supabase, then return here and sign in.",
          );
          setMode("signin");
        }
      }
    ...
    >
      ...
      >
        ...
        <Text style={styles.title}>
          Everything your child needs, between two homes.
        </Text>
        <Text style={styles.body}>
          Private shared planning for handovers, belongings, school dates and
          medical essentials.
        </Text>
        ...
        <View style={styles.demo}>
          <SecondaryButton
            label="Open local demo instead"
            onPress={onUseDemo}
          />
        </View>
        <Text style={styles.privacy}>
          Real shared records are protected by Supabase authentication and
          household-level Row Level Security.
        </Text>
      </ScrollView>
    </KeyboardAvoidingView>
  );
