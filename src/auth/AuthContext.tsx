...
function getAuthParameters(url: string): URLSearchParams {
  ...
  if (hash) {
    ...
    hashParameters.forEach((value, key) => { if (!parameters.has(key)) parameters.set(key, value); });
  }
  return parameters;
}

async function createSessionFromUrl(url: string): Promise<void> {
  if (!supabase || !url.startsWith('homebridge://')) return;
  const parameters = getAuthParameters(url);
  const errorDescription = parameters.get('error_description') ?? parameters.get('error');
...
export function AuthProvider({ children }: PropsWithChildren) {
  ...
  useEffect(() => {
    ...
    const handleUrl = (url: string | null) => {
      if (!url) return;
      createSessionFromUrl(url).catch((error: unknown) => console.error('HomeBridge authentication link failed:', error));
    };
  ...
  const value = useMemo<AuthContextValue>(() => ({
    ...
    signUp: async (email, password, displayName) => {
      ...
      const { data, error } = await supabase.auth.signUp({
        email: email.trim(),
        password,
        options: { emailRedirectTo: AUTH_REDIRECT_URL, data: { display_name: displayName.trim() } },
      });
      if (error) throw error;
