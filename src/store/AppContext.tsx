...
const DEMO_STORAGE_KEY = '@homebridge/demo-state-v4';
const liveStorageKey = (householdId: string) =>
  `@homebridge/live-cache-v4/${householdId}`;
...
async function loadLiveState(workspace: Workspace): Promise<AppState> {
  ...
  const overrideRows = (overridesResult.data ?? []) as Array<Record<string, any>>;
  ...
  const nextTo = parentFromHousehold(
  ...
  const pickupLocation =
    handoverRow?.pickup_location ?? 'the agreed handover point';
  ...
  return {
    ...
    medicalItems,
    careScheduleRules: scheduleRows.map((row) => ({
      id: row.id,
      title: row.title,
      startsOn: row.starts_on,
      householdLabel:
        row.household_label === "Mum's house"
          ? "Mum's house"
          : "Dad's house",
    ...
    careOverrides: overrideRows.map((row) => ({
      id: row.id,
      startsOn: row.starts_on,
      endsOn: row.ends_on,
      householdLabel:
  ...
  const value = useMemo<AppContextValue>(() => ({
    ...
    deleteHandoverTask: async (id) => {
      if (!workspace || !supabase) {
        setState((current) => ({
          ...current,
          handoverTasks: current.handoverTasks.filter((task) => task.id !== id),
        }));
        return;
      }
    ...
    removeMember: async (userId) => {
      ...
      if (workspace.role !== 'owner') {
        throw new Error('Only the household owner can remove another member.');
      }
      if (userId === workspace.userId) {
        throw new Error('The household owner cannot remove their own account.');
      }
